test_that("encode_yaml round-trips basic R lists", {
  obj <- list(
    foo = "bar",
    baz = list(TRUE, 123L),
...
3.5: d
  ...
  encoded <- encode_yaml(parsed)
  reparsed <- parse_yaml(encoded)
  expect_identical(reparsed, parsed)
})

test_that("encode_yaml returns visibly", {
  expect_visible(encode_yaml(list(answer = 42L)))
  out <- encode_yaml(list(answer = 42L))
...
test_that("encode_yaml retains partial names as mapping keys", {
  obj <- list(a = 1L, 2L)
  encoded <- encode_yaml(obj)
  reparsed <- parse_yaml(encoded)
...
test_that("encode_yaml converts NA names to null YAML keys", {
  obj <- list(a = 1L, b = 2L)
