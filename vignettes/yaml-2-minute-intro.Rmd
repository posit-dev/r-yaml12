...
If `yaml` already gives you the behavior and compatibility you need,
there is no need to switch. Use `yaml12` when your YAML producers and
consumers use 1.2 or when you want the core schema rules above. Since
`yaml12` also emits YAML 1.2, strings such as `yes` may be written
without quotes; test that boundary if a YAML 1.1 reader will consume the
result.
