...
The presence of a custom tag bypasses normal scalar node typing
inferencing; the scalar is always returned as a string even when the
content looks like another type.
...
The `!!timestamp` and `!!binary` tags are occasionally useful, but the
logic for handing richer types is encouraged to live at the application
level, and not in the core schema.
...
```{r}
# Timestamp handler: date-only stays Date, otherwise try common POSIX formats,
...
# Binary handler: decode Base64 into raw; leave as-is on failure.
